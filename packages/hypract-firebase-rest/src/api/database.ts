...
const queryToUrlParams = (query?: Query): string => {
 ...
 */
export const query = (path: string, query?: Query): Promise<any> => {
  return fetch(`${conf.databaseURL}/${path}.json?${queryToUrlParams(query)}`).then(handleFirebaseResponse)
}
